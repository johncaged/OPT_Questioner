...
def main():
    ...
    model = BaseQuestioner(tokenizer, MultiStageAdapter(), auto_regressive=True, use_img_region_embedding=True)
    # model = BaseQuestioner(tokenizer, MultiStageAdapter(), auto_regressive=True, use_img_region_embedding=False)
    model = ModelWrapper(model)
    checkpoint = torch.load('./log/vg_224_512_embedding_new/checkpoint/best.pth', map_location='cpu')
    # checkpoint = torch.load('./log/vg_224_512/checkpoint/best.pth', map_location='cpu')
    ...
    types = [
        *[{'answer_type': 'what'} for _ in range(3)],
        *[{'answer_type': 'how'} for _ in range(2)],
        *[{'answer_type': 'where'} for _ in range(2)],
    ...
    for i, batch in enumerate(val_dataset):
        ...
        for k in range(object_sample):
            ...
            object_embeddings = batch[0]['object_embeddings']
            if 'video_input' in batch[0]:
                del batch[0]['video_input']
            
            def sample(_object_items):
                _object_items = list(zip(*_object_items))
                return _object_items[min(k, len(_object_items) - 1)]

            sampled_objects = [sample(_objects) for _objects in zip(objects, object_embeddings)]
            # set region embedding
            sampled_object_embeddings = torch.stack([_object[1] for _object in sampled_objects])
            batch[0]['region'] = sampled_object_embeddings
            batch[0]['caption_region'] = sampled_object_embeddings
            # generate region captions
            processed_sampled_objects = [process_tip(_object[0]) for _object in sampled_objects]
            ...
            processed_tip = torch.cat([process_tip(_object[0], region_caption) for _object, region_caption in zip(sampled_objects, captions)], dim=0)
            ...
            for j in range(repeat_sample):
                ...
                for answer, question, prob, img_id, _object, _caption in zip(convert_items(detach(prediction), tokenizer, start_token=tokenizer.question_answer_sep_token),
                    ...
                    _data = {
                        ...
                        'object': convert_object(_object[0]),
