...
class BertEmbeddings(nn.Module):
    ...
    """
    def __init__(self, config,visual_config):
        ...
        self.visual_token_embeddings = nn.Embedding(visual_config.visual_vocab_size, visual_config.hidden_size)
        self.video_token_position_embeddings = nn.Embedding(visual_config.video_token_max_position_embeddings, visual_config.hidden_size)
        self.image_token_position_embeddings = nn.Embedding(visual_config.image_token_max_position_embeddings, visual_config.hidden_size)
        self.image_token_type_embeddings = nn.Embedding(1, visual_config.hidden_size)
        self.video_token_type_embeddings = nn.Embedding(1, visual_config.hidden_size)
        #self.visual_frame_embeddings = nn.Embedding(config.max_visual_frame_embeddings, config.hidden_size)
        self.tokens_per_frame = visual_config.tokens_per_frame
        self.LayerNorm_visual = BertLayerNorm(visual_config.hidden_size, eps=1e-12)
        
    def forward(self, input_ids, token_type = None):
        # import ipdb 
        # ipdb.set_trace()
        ...
        elif token_type == 'image':   ###input_ids b,256  [BOI],1,2,...,256

            b,n = input_ids.shape
            token_type_ids = torch.zeros_like(input_ids)
            token_type_embeddings = self.image_token_type_embeddings(token_type_ids)
            token_embeddings = self.visual_token_embeddings(input_ids)
            position_ids = torch.arange(n-1, dtype=torch.long, device=input_ids.device)
            position_ids = position_ids.unsqueeze(0).expand(b,-1)
            frame_ids = torch.zeros_like(input_ids[:,1:])
            #frame_embeddings = self.visual_frame_embeddings(frame_ids)
            position_embeddings = self.image_token_position_embeddings(position_ids)
            embeddings = token_embeddings + token_type_embeddings 
            embeddings[:,1:] = embeddings[:,1:] + position_embeddings #+ frame_embeddings
            embeddings = self.LayerNorm_visual(embeddings)
            embeddings = self.dropout(embeddings)
            return embeddings

        elif token_type == 'video':   ###input_ids b,256  [BOV],1,2,...,64,1,2,...,64
            b,n = input_ids.shape
            token_type_ids = torch.zeros_like(input_ids)
            token_type_embeddings = self.video_token_type_embeddings(token_type_ids)
            token_embeddings = self.visual_token_embeddings(input_ids)
            ahead_frames, cur_pixels = (n-1)//self.tokens_per_frame, (n-1)%self.tokens_per_frame
            position_ids = [i for i in range(self.tokens_per_frame)]*ahead_frames + [i for i in range(cur_pixels)]
            position_ids = torch.tensor(position_ids).to(input_ids).unsqueeze(0).expand(b,-1)
            position_embeddings = self.video_token_position_embeddings(position_ids)           
            frame_ids = []
            for i in range(ahead_frames):
                frame_ids += [i]*self.tokens_per_frame
            frame_ids += [ahead_frames] * cur_pixels
            frame_ids = torch.tensor(frame_ids).to(input_ids).unsqueeze(0).expand(b,-1)
            #frame_embeddings = self.visual_frame_embeddings(frame_ids)
            embeddings = token_embeddings + token_type_embeddings 
            embeddings[:,1:] = embeddings[:,1:] + position_embeddings #+ frame_embeddings
            embeddings = self.LayerNorm_visual(embeddings)
...
class BertCrossAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = nn.Linear(config.hidden_size, self.all_head_size)
        self.key = nn.Linear(config.key_value_dim, self.all_head_size)
        self.value = nn.Linear(config.key_value_dim, self.all_head_size)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, cross_hidden_states, use_cache=False, cache=None,
    ...
    def forward(self, hidden_states, attention_mask, cross_hidden_states,use_cache=False,cache=None,
        ...
        for idx, layer_module in enumerate(self.layer):
            if self.checkpointing:
                # print(1)
                hidden_states,cache = torch.utils.checkpoint.checkpoint(layer_module, hidden_states, attention_mask, cross_hidden_states,use_cache,cache,
                                        cache_first, idx, cache_type)
            else:
                hidden_states,cache = layer_module(hidden_states, attention_mask, cross_hidden_states,use_cache,cache,
                                        cache_first, idx, cache_type)
...
class BertModel(PreTrainedBertModel):
    ...
    def __init__(self, config,visual_config):
        ...
        self.embeddings = BertEmbeddings(config,visual_config)
        self.encoder = BertEncoder(config)
        self.pooler = BertPooler(config)
        self.has_cross_attn = config.has_cross_attn
        if visual_config.has_decoder:
            visual_config.has_cross_attn=False
            if config.decoder_use_bert:
                self.decoder = BertEncoder(visual_config)
            else:
                self.decoder = OPTEncoder(visual_config, mode='prenorm')
    ...
    def forward(self, tokens, task_prompt = None, video_feat = None, audio_feat=None, \
        ...
        if (not self.has_cross_attn) or (self.has_cross_attn and not use_cross_attn):
            ...
            if not use_cache or (use_cache and cache_first):
                ...
                if task_prompt is not None:
                    if forward_type=='encoder':
                        ...
                        # import ipdb 
                        # ipdb.set_trace()
                        input_feat = torch.cat((input_feat, task_prompt),dim=1)
                        attention_mask_prompt = torch.ones(*task_prompt.shape[:2]).to(attention_mask)
                        attention_mask = torch.cat((attention_mask, attention_mask_prompt),dim=1)  
            ...
            if audio_feat is not None:
                cross_hidden_states.append(audio_feat)
            if cross_hidden_states != []:
